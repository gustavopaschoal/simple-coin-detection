...
# creating kernels to be used as structuring elements in morphological operations
kernel = np.ones((1,2), np.uint8) 
kernel2 = cv.getStructuringElement(cv.MORPH_ELLIPSE,(20,20))

# eroding initial gray image to get rid of noise in the interior region of coins
erosion = cv.erode(input_dolar_bin,kernel,iterations = 3)

# dilating result to homogenize background with circular kernel to smooth coins edges
...
# getting blobs data
keypoints = detector.detect(erosion2)

# number of coins (blobs) detected
print('Number of coins detected: ', len(keypoints))

# drawing coins (blobs) borders
borders = cv.drawKeypoints(erosion2, keypoints, np.array([]), (0,0,255), cv.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)

# drawing circles in original image using data from blob detection
output = input_dolar.copy()
for i in range(len(keypoints)):
    cv.circle(output, (int(keypoints[i].pt[0]), int(keypoints[i].pt[1])), int(keypoints[i].size/2), (0, 255, 0), 4)
    cv.circle(output, (int(keypoints[i].pt[0]), int(keypoints[i].pt[1])), 1, (255, 0, 0), 5)

# creating folder to save result if not already created
...
cv.imshow('bin image with morph filtering', erosion2)
